...
def call_postprocessing_1(**kwargs):
    ...
    if not res.ok:
        gemslog(LogLevel.ERROR, res.text, order_id=pid, service_name=f"task1_stitching_3_{kwargs['task_instance'].task_id}")
...
dag = DAG(
    ...
    description='stitching dag offset by 20 seconds',
