...
def call_grid_service(**kwargs): 
    ...
    pid = f"{kwargs['dag_run'].run_id}_{pu}_{kwargs['task_instance'].try_number}_applymodelfast"
    ...
    if not res.ok:
        gemslog(LogLevel.ERROR, res.text, order_id=pid, service_name=f"task2_applymodelfastlane_{kwargs['task_instance'].task_id}")
...
def call_calc_feature_s1(s1_bands, pid, **kwargs):
    ...
    for band in s1_bands:
        ...
        if direction_band[0].upper() == "ASC":
            ...
            gemslog(LogLevel.ERROR, f"{band} not in the correct format", order_id=pid, service_name=f"task2_applymodelfastlane_{kwargs['task_instance'].task_id}")
...
def call_calc_feature_s2(s2_bands, pu, pid, **kwargs):
    ...
    data_filter_s2 = json.loads(get_data_from_payload("data_filter_s2", str, True, **kwargs))
    ...
    res = requests.post('http://calc-feature-service-prod-t2.task-2-production:80/temporals2features', 
                        params = params, data = data, headers = headers, timeout = 60)
...
def create_calc_feat_data(start_date, end_date, bands, interval_size, coordinates, data_filter=None):
    ...
    if data_filter:
        data = {"aoi": coordinates, "features": features, "data_filter": data_filter}
    else:
...
dag = DAG(
    ...
    description='task2 apply model fast lane',
