...
def call_postprocessing_1(**kwargs):
    ...
    if not res.ok:
        gemslog(LogLevel.ERROR, res.text, order_id=pid, service_name=f"task1_stitching_4_{kwargs['task_instance'].task_id}")
...
dag = DAG(
    ...
    description='stitching dag offset by 30 seconds',
    schedule_interval=None,#'*/1 * * * *' if datetime.now().weekday() < 5 else '*/1 * * * *',  # UTC time (this is helpful to create the schedule: https://crontab.guru/) 16-23,0-9
