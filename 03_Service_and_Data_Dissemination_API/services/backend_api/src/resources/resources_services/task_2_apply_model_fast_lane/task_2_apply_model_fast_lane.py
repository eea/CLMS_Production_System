...
from models.models_services.task_2_apply_model_fast_lane.task_2_apply_model_fast_lane_model import t2_apply_model_fast_lane_request_model
...
class Task2ApplyModelFastLane(Resource):
    ...
    This class defines the API call for starting the apply model fast lane (Task 2) workflow. The class consists of one
    ...
    def post(self):
        """ POST definition for calculating features and applying models for CLC+ task 2 (fast lane)
        ...
        try:
            ...
            if not payload_check[0]:
                ...
                gemslog(LogLevel.WARNING, f"'message': {error.to_dict()}", 'API-apply_model', order_id)
            ...
            gemslog(LogLevel.INFO, f'Request payload: {req_args}', 'API-apply_model', order_id)
            ...
            error = ServiceUnavailableError(f'Could not connect to the database server: {err}', '', '')
            gemslog(LogLevel.ERROR, f"'message': {error.to_dict()}", 'API-apply_model', order_id)
