...
class DeleteCustomerById(Resource):
    ...
    def delete(self, user_id):
        ...
        db_query_api = "UPDATE customer.customer SET deleted_at = NOW() WHERE customer_id = %s"
        db_query_oauth = "UPDATE public.oauth2_client SET deleted_at = NOW() WHERE client_id = %s"
        db_query_token = "UPDATE public.oauth2_token SET deleted_at = NOW() WHERE client_id = %s"

        try:
            gemslog(LogLevel.INFO, f'Request path parameter: {user_id}', 'API-delete_customer_by_id')

            execute_database(db_query_api, (user_id,), database_config_file, database_config_section_api, True)
            execute_database(db_query_oauth, (user_id,), database_config_file, database_config_section_oauth, True)
            execute_database(db_query_token, (user_id,), database_config_file, database_config_section_oauth, True)

        except KeyError as err:
            error = BadRequestError(f'Key error resulted in a BadRequest: {err}', '', traceback.format_exc())
            gemslog(LogLevel.WARNING, f"'message': {error.to_dict()}", 'API-delete_customer_by_id')
            return {'message': error.to_dict()}, 400

        except AttributeError:
            error = ServiceUnavailableError('Could not connect to the database server', '', '')
            gemslog(LogLevel.ERROR, f"'message': {error.to_dict()}", 'API-delete_customer_by_id')
            ...
            gemslog(LogLevel.INFO, f'User ID {user_id} has been deleted', 'API-delete_customer_by_id')
            return '', 204
