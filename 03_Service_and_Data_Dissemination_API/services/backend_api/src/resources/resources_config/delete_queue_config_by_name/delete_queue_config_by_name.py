...
class DeleteQueueByName(Resource):
    ...
    This class defines the API call for the delete queue configuration by name script. The class consists of one method
    ...
    def delete(self, queue_name):
        ...
        queue configuration by name script. It returns no message body and thus no contents. In contrast it returns the
        ...
        try:
            gemslog(LogLevel.INFO, f'Request path parameters: {queue_name}', 'API-delete_queue_config_by_name')

            db_query = "UPDATE msgeovilleconfig.message_queue_config SET deleted_at = NOW() WHERE queue_name = %s"
            execute_database(db_query, (queue_name,), database_config_file, database_config_section_api, True)

        except AttributeError:
            error = ServiceUnavailableError('Could not connect to the database server', '', '')
            gemslog(LogLevel.ERROR, f"'message': {error.to_dict()}", 'API-delete_queue_config_by_name')
            ...
            gemslog(LogLevel.INFO, f'Deleted queue config for: {queue_name}', 'API-delete_queue_config_by_name')
            return '', 204
