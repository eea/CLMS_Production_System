...
class DeleteQueueConfiguration(Resource):
    ...
    This class defines the API call for the delete message queue configuration script. The class consists of one method
    ...
    def delete(self):
        ...
        <p style="text-align: justify">This method defines the handler for the DELETE request of the message delete
        queue configuration script. It returns no message body and thus no contents. In contrast it returns the HTTP
        status code 204.</p>
        ...
        try:
            db_query = "UPDATE msgeovilleconfig.message_queue_config SET deleted_at = NOW()"
            execute_database(db_query, (), database_config_file, database_config_section_api, True)

        except AttributeError:
            error = ServiceUnavailableError('Could not connect to the database server', '', '')
            gemslog(LogLevel.ERROR, f"'message': {error.to_dict()}", 'API-delete_queue_config')
            ...
            gemslog(LogLevel.INFO, f'Successfully deleted the queue configuration table', 'API-delete_queue_config')
            return '', 204
